function [Packing,geometricInfo] = AnalyzeSTL(STL_filename,magnetization_data_file, n_particles, STLtoMagTenseSCALE, workingDir,ScaleFactor_Distancing)
...
Packing = processParticleTypes(magnetization_data_file,STLtoMagTenseSCALE,Packing);
end
...
function Packing = processParticleTypes(magnetization_data_file,STLtoMagTenseSCALE,Packing)
  load([magnetization_data_file,'.mat'],'Radii', 'Heights') ;
  [RR1,RR2] = ndgrid(Radii(:),STLtoMagTenseSCALE.*Packing.AllTheRadii) ;
  [DD1,DD2] = ndgrid(Heights(:),STLtoMagTenseSCALE.*Packing.AllTheHeights) ;
  AllDists = sqrt((RR1-RR2).^2+(DD1-DD2).^2) ;
  [~,iMin] = min(AllDists,[],1) ;
  [Packing.iRadius,Packing.iHeight] = ind2sub(size(Radii),iMin) ;
  [Uniques,~,AllTheTypes] = unique(Packing.iRadius+1i.*Packing.iHeight) ;
  Packing.AllTheTypes = AllTheTypes ;
  Packing.UniqueCombinationsRadii = (real(Uniques)) ;
  Packing.UniqueCombinationsHeights = (imag(Uniques)) ;
end